...
any rational number $\epsilon>0$, 
by a result of Viehweg (c.f. \cite{V1}, \cite{V2}, see also \cite{Ko2} \S 10),
we have that the homomorphism
$$
\phi _{k,\epsilon H}:
H^0(X,\OO _X(k(NK_X+a^*\epsilon H)))\lra H^0(F,\OO _F
(kNK_F))$$ is surjective for some $k> 0$.
Fix one such $k$ and fix $D_0$ any reduced irreducible component of $D$.
Let $$V _{l,\epsilon H}^{D_0}:=H^0(X,\OO _X
(lk(NK_X+a^*\epsilon H))\ot \II _{D_0}^{<l>})$$
where $\II ^{<l>}_{D_0}$ denotes the $l^{th}$ {\it symbolic power} of $\II _{D_0}$
i.e. the ideal defined by the germs of functions with multiplicity
at least $l$ at a general point of $D_0$.
%$|V _{l,\epsilon H}^D|:=\phi _{l,\epsilon H }^{-1}(lD)$ where, by abuse 
%of notation, $\phi _{l , \epsilon H}$ also denotes the corresponding map of linear series. 
Notice that for all $j>0$, there is a natural morphism
$|V^{D_0}_{l, \epsilon H}|^{\times j}\lra |V^{D_0}_{lj,\epsilon H}|$.
Moreover, for all $l$, we have
$l\cdot \phi _{k,\epsilon H}^{-1}(kD)\subset |V _{l,\epsilon H}^{D_0}|$ and so 
$V _{l,\epsilon H}^{D_0}\ne 0$.
Therefore, we may consider the graded linear series
$V^{D_0}_{\bullet, \epsilon H}:=\{ V^{D_0}_{l,\epsilon H} \}$
and, for any integer $c>0$, the corresponding asymptotic multiplier ideal sheaf
$$\II ^{c\cdot D_0}_{\epsilon H}:=\II (X,c\cdot V^{D_0}_{\bullet, \epsilon H})
.$$
By Section \ref{MIS}, $\II ^{c\cdot {D_0}}_{\epsilon H}=
\II (\frac{c}{l}|V^{D_0}_{l,\epsilon H}|) $
for all $l\gg 0$ sufficiently divisible.
We now let $c=q+1$. Sections of $V^{D_0}_{l,\epsilon H}$ have multiplicity at least
$l$ along $D_0$. Since the codimension of $D_0$ in $X$ is $q+1$,
by Proposition \ref{ideal},
one has that $$\II ^{c\cdot {D_0}}_{\epsilon H}\subset \II _{D_0}.$$
Let $$\FF _\epsilon :=a_*(\omega _X ^{\ot 1+ckN}\ot 
\II ^{c\cdot {D_0}}_{\epsilon H}).$$
We claim that there exists a rational number $1<\epsilon <\frac {1}{ck}$
...
linear series:
$$|V^{{D_0}}_{l,\epsilon ' H}|\times a^*|lk(\epsilon -\epsilon ')H|\lra
|V^{D_0}_{l,\epsilon  H}|$$
and since $a^*|lk(\epsilon -\epsilon ')H|$ is base point free, 
by Proposition \ref{ideal2}, one has
$$\FF _\epsilon\subset\FF _{\epsilon '}
\ \ \ for\ all\ 0<\epsilon '<\epsilon .$$ 
%Let $\nu :\tilde{X}\lra X$ be a log resolution of $|V^D_{l, \epsilon  H}|$
%so that $\nu ^*|V^D_{l,\epsilon  H}|=|M|+F$ where $|M|$ is base point 
%free and $F$ has simple normal crossings support.
%If $l$ is sufficiently big and divisible, then $\II ^{c\cdot D}_{\epsilon H}=
%\nu _* \OO _{\tilde{X}}(K_{\tilde{X}/X}-\lfloor \frac {c}{l}F\rfloor)$.
%Let $\tilde {f}=f\circ \nu$, then for any integer $t>0$
%$$K_{\tilde{X}}+cN\nu ^* K_X -\lfloor 
%\frac {c}{l}F\rfloor +t\tilde{f}^*H\equiv \{ \frac {c}{l}F\} +\frac {c}{l}M+
%(t-c\epsilon)\tilde {f}^*H.$$
Since $\FF _\epsilon:= a_*(\omega _X \ot M \ot \II (\Delta ))$
where $M-\Delta\equiv{ -ck\epsilon }a^*H$ 
and $\Delta\equiv ck(NK_X+\epsilon a^*H)$ with 
$ck\epsilon<1$, 
...
$\hat{A}$ and pick $0<\epsilon ' \ll 1$ so that $\phi _L^*H-k\epsilon ' H$
...
Since $\kappa (X)=0$, we have that $h^0(\omega _X^{\ot 1+ckN})=1$ 
and so $D_0\subset Bs |(1+ckN)K_X|$. 
Since $P_N(F)\geq 2$, then the divisors $D_0\subset D\in |NK_F|$ cover
$F$ and so $F\subset Bs|(1+ckN)K_X|$ but this is impossible as $F$ is a general
fiber.
